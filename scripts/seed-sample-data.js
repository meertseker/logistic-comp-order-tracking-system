...
// Electron'un userData klasörünü simüle et
const userDataPath = join(homedir(), 'AppData', 'Roaming', 'Sekersoft');
const dbPath = process.env.DB_PATH || join(userDataPath, 'transport.db');

console.log('📊 Örnek veri ekleme scripti başlatılıyor...');
...
const statusWeights = [0.2, 0.3, 0.4, 0.1]; // Çoğu teslim edilmiş olsun
...
for (let i = 0; i < 30; i++) {
  ...
  routes.push({ from, to, distance, hgsCost, time });
...
for (let i = 0; i < 50; i++) {
  ...
  // Gerçek fiyat (bazen önerilenin altında olabilir)
